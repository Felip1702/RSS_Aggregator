...
document.getElementById('addFeed').addEventListener('click', () => {
  ...
  if (feedCount < 5) {
    ...
    newLabel.innerText = `Feed ${feedCount + 1}:`; // Correção: template literal
    const newInput = document.createElement('input');
    newInput.type = 'url';
    newInput.id = `feed${feedCount + 1}`; // Correção: template literal
...
document.getElementById('rssForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/aggregate`, { // Correção: template literal
      method: 'POST',
...
document.getElementById('generateRss').addEventListener('click', async () => {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/generate-rss`, { // Correção: template literal
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ feeds }),
    });
